...
它是跨平台 Agent 管理框架 - 统一对接 OpenClaw、ZeroClaw 等 Agent 框架，然后再在上面加一层应用框架，方便用户所见即所得创建 AI 数字员工等智能体。而不需要管理底层配置，只需要在应用层配置即可。

简单来说：

OpenClaw、ZeroClaw作为底层，负责“连接”和“能力”——它封装了大模型（API调用、提示工程、上下文管理），并整合了本地Agent所需的通用组件（如记忆存储、工具调用框架、基础规划逻辑）。这一层相当于Agent的“骨架”或“操作系统”，与具体业务无关。

AgentBrother 则搭建在 OpenClaw 之上，聚焦“应用”和“场景”——它利用底层提供的Agent能力，封装成面向特定业务逻辑的代理模块（如客服Agent、数据分析Agent、工作流编排等），可能还包含业务知识库、定制化行为策略等。这一层更像是“血肉”，让Agent真正解决实际问题。

这种分层设计的好处是职责清晰：底层专注技术与通用能力，上层专注业务与快速迭代。两者通过明确接口协作，既保持了灵活性，又降低了耦合。

## 项目目的

龙哥的核心目标是为用户提供一个统一的、跨平台的界面，用于集成各种 AI Agent 框架，如 OpenClaw 和 ZeroClaw。通过龙哥，用户可以：

- 统一管理多个 Agent 框架，无需在不同工具之间切换
- 所见即所得地创建、配置和使用 AI 数字员工
- 创建虚拟组织，创建项目，自动推动项目进度和发展
- 在不同平台（Mac、Windows、Web、手机）上获得一致的使用体验
- 简化 Agent 的创建和管理流程，降低使用门槛

龙哥不只是为了管理龙虾和螃蟹，更是为了让普通用户轻松创作出好玩、有趣、有用的 AI 代理数字人、数字宠物等，也为了企业更智能推动项目业务发展。我们给每个 Agent 赋予独特的情感和个性，让它们成为你生活中的得力助手和伙伴！
...
**（声明：这是我自己玩的项目，不提供任何支持或保证，是个半吊子想法。如果使用问题或建议，欢迎联系我。谢谢。）**
